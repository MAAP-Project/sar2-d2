...
from cum_sum_dist import reader_ni, util, filter_SAR, generate_log
from cum_sum_dist.dist_runconfig import (
    _get_parser,
    RunConfig,
)
...
def get_rtc_stack_block(filename_list, blocksize, block_ind, scale="db"):
    ...
    for find, fname in enumerate(filename_list):
        ...
        target_rtc_image = band.ReadAsArray(
            int(target_ul_x_ind), int(target_ul_y_ind), int(col_sub), int(row_sub)
        )
...
def process_row(row, polarizations, filter_option, block_param):
        ...
        if file_path is not None:
            if os.path.exists(file_path):
                ...
                da = xr.DataArray(
                    ...
                    dims=["y", "x"],
                    coords={
                        "y": np.arange(subset_data.shape[0]),
    ...
    da_polarized = xr.concat(
        polarization_stack, dim=pd.Index(actual_pol, name="polarization")
    )
    return da_polarized
...
def dist_workflow(cfg):
    ...
    for block_ind, block_param in enumerate(block_params):
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    process_row, row, polarizations, filter_option, block_param
                )
        ...
        Xmean_maxmin = (X.max(dim="time") + X.min(dim="time")) / 2.0
        R = X - Xmean_maxmin
        ...
        cumsum_result_diff = cumsum_result_masked.max(
            dim="time", skipna=True
        ) - cumsum_result_masked.min(dim="time", skipna=True)
        ...
        for pol_single in polarizations:
            ...
            for time_step in R["time"]:
                ...
                cumsum_result_date = cumsum_result.sel(time=time_step).sel(
                    polarization=pol_single
                )

                # Compute the mean value of S_date over non-zero data
                mean_S_date = cumsum_result_date.mean(
                    dim=("x", "y"), skipna=True
                ).item()
                mean_X_date = (
                    X.sel(time=time_step)
                    .sel(polarization=pol_single)
                    .mean(dim=("x", "y"), skipna=True)
                    .item()
                )
                mean_R_date = (
                    R.sel(time=time_step)
                    .sel(polarization=pol_single)
                    .mean(dim=("x", "y"), skipna=True)
                    .item()
                )
                ...
                if np.isnan(mean_X_date):
                    print(
                        f"Mean value of S for date {date_str} and polarization {pol_single}: {mean_S_date} {mean_X_date} {mean_R_date}"
                    )
                    print(
                        f"No data for date {date_str} and polarization {pol_single}, skipping."
                    )
            ...
            if proc_param.threshold is None:
                ...
                print(
                    f"Setting the threshold for cumsum_result_Diff based on the {quantile}th percentile to: {dthres:.3f}"
                )
            ...
            if proc_param.debug_mode:
                change_path_name = (
                    f"{scratch_dir}/cumsum_maximum_binary_{pol_single}.tif"
                )
                print(f"saving {change_path_name}")
                util.write_raster_block(
                    change_path_name,
            ...
            cumsum_result_Diff_masked = (
                cumsum_result_max_masked - cumsum_result_min_masked
            )
            ...
            if proc_param.debug_mode:
                ...
                confidence_valid_path_name = (
                    f"{output_dir}/confidence_valid_{pol_single}.tif"
                )
                util.write_raster_block(
                    confidence_valid_path_name,
                ...
                confidence_invalid_path_name = (
                    f"{output_dir}/confidence_invalid_{pol_single}.tif"
                )
                util.write_raster_block(
                    confidence_invalid_path_name,
            ...
            cumulative_change_path_name = (
                f"{output_dir}/cumulative_change_by_year_{pol_single}.tif"
            )
